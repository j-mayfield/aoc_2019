...
class SetAndForget
  def initialize(input)
    @ascii = IntCodeProcessor.new(input, quiet: true)
  end

  def sum_of_alignment_params
    all_intersections.map { |i| i[:x] * i[:y] }.inject(:+)
  end

  def all_intersections
    ascii_lines.each_index.map do |y|
      ascii_lines[y].each_index.map do |x|
        { x: x, y: y } if intersection?(x, y)
      end
    end.flatten.reject(&:nil?)
  end

  def scaffold?(x, y)
    off_the_map?(x, y) ? false : ascii_lines[y][x].eql?(35)
  end

  def off_the_map?(x, y)
    ascii_lines[y].nil? || ascii_lines[y][x].nil?
  end

  def intersection?(x, y)
    [scaffold?(x, y), scaffold?(x, y - 1), scaffold?(x + 1, y), scaffold?(x, y + 1), scaffold?(x - 1, y)].inject(:&)
  end

  def ascii_lines
    @ascii_lines ||= run_ascii.chunk { |i| i.eql?(10) }.map { |i| i[1] unless i[0] }.reject(&:nil?).to_a
  end
  ...
  def draw
    ascii_lines.each do |line|
      puts line.join.gsub(/35/, '#').gsub(/46/, ' ').gsub(/94/, '^').gsub(/62/, '>').gsub(/118/, 'v').gsub(/60/, '<')
      #puts line.map { |i| [i.to_s].pack('h*') }.join
    end
  end
end

puts 'day_17 part_1'
puts SetAndForget.new(File.read('day_17_input.txt')).sum_of_alignment_params
SetAndForget.new(File.read('day_17_input.txt')).draw
